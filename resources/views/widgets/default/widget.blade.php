...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
   <head>
	...
	<style type="text/css">
	   ...
	   .btn-primary{
		   background-color: {{ get_company_option('primary_color','#556ee6') }} !important;
		   color: {{ get_company_option('label_color','#FFFFFF') }};
	       border: none;
		   border-radius:0px;
	   }
   ...
   <body>
	<div class="panel panel-default live_chat_widget">
					  ...
					  @if(get_company_option('allow_department') == "yes")
						  <div class="form-group">
							...
							<select class="form-control select2" name="department" id="department" required>
							   ...
							   {{ create_option('departments','id','department',array("company_id",helpdesk_decrypt($_GET['token']))) }}
							</select>
						  </div>
					  @endif
					  <div class="form-group">
	...
	function playSound(){
		if(localStorage.getItem("mute") !="yes"){
			...
			var audio = document.createElement('audio');
			  ...
			  audio.src = "{{ asset('sounds/'.get_option('message_sound')) }}"
			  audio.autoplay = true;
